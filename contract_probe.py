...
+            if isinstance(node.target, ast.Name) and node.target.id == "ZONE_THRESHOLDS" and node.value is not None:
+                try:
+                    return ast.literal_eval(node.value), None
+                except Exception as e:
+                    return None, f"literal_eval_failed: {type(e).__name__}: {e}"
...
+    fx: Dict[str, Any] = {
+        "instrument_has_ZONE_THRESHOLDS": marker_line is not None,
