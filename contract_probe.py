...
def _extract_thresholds_internal(src: str) -> Optional[Dict[str, Any]]:
    """
    Extraction robuste des seuils de zones, sans dépendance externe.
    Supporte:
      - AST: ZONE_THRESHOLDS = [0.5, 1.5, 2.5] / (...) / AnnAssign
      - Regex tolérante (fallback)
    Retour:
                ...
                for t in node.targets:
                    if isinstance(t, ast.Name) and t.id == "ZONE_THRESHOLDS":
                        ...
                        if isinstance(val, (list, tuple)) and all(isinstance(x, (int, float)) and not isinstance(x, bool) for x in val):
                            return {"thresholds": [float(x) for x in val], "pattern": "ast_assign"}
            ...
            if isinstance(node, ast.AnnAssign):
                ...
                if isinstance(t, ast.Name) and t.id == "ZONE_THRESHOLDS" and node.value is not None:
                    try:
                        ...
                        val = None
                    if isinstance(val, (list, tuple)) and all(isinstance(x, (int, float)) and not isinstance(x, bool) for x in val):
                        return {"thresholds": [float(x) for x in val], "pattern": "ast_annassign"}

    except Exception:
        # on tente regex
        pass

    # 2) Regex tolérante (pas d'ancrage strict de fin de ligne)
...
def _cli_section(instrument_path: Path) -> Dict[str, Any]:
...
def _zones_section(instrument_path: Path) -> Dict[str, Any]:
    src = instrument_path.read_text(encoding="utf-8", errors="ignore")
    has_marker = "ZONE_THRESHOLDS" in src

    extracted = _extract_thresholds_internal(src)
    out: Dict[str, Any] = {
        "zones": {},
        "constants": {},
        "if_chain": [],
    ...
    ths = extracted.get("thresholds") or []
    out["method"] = extracted.get("pattern", "internal")
    out["pattern"] = extracted.get("pattern", "internal")
    out["constants"] = {f"THRESH_{i}": float(v) for i, v in enumerate(ths)}
    out["zones"] = dict(out["constants"])
    out["_forensics"] = {
        "instrument_has_ZONE_THRESHOLDS": has_marker,
        "thresholds_len": len(ths),
    }
    return out
...
def main() -> int:
    ...
    cli = _cli_section(instrument_path)
    zones = _zones_section(instrument_path)
